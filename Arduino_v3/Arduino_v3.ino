  ...
  if (pspSerial.available() > 3)
  {
    ...
    if (pspdata[0] == 0x5A)
    {
      ...
      case 0x90:
      {
        ...
        AES aes;
        aes.set_key(tempKey, 16);
        aes.cbc_decrypt(req, payload, 32, dec_iv_B);

        memcpy(payload91, payload + 8, 8);    // payload[8:16]
        memcpy(payload91 + 8, payload, 8);    // payload[0:8]
        memset(payload91 + 16, 0, 16);        // bytearray(16)
        memcpy_P(temp, go_secret, 16);

        for (uint8_t i = 16; i < 32; i++)
          if (payload[i] != temp[i-16])
          {
            debugSerial.print(F("Invalid request from Syscon"));
            return;
          }
//
        ...
        AES aes2;
        aes2.set_key(tempKey, 16);
        aes2.cbc_decrypt(payload91, resp2, 32, dec_iv_B);
//
        memcpy_P(tempBuffer, answer_90, 11);
        memcpy(tempBuffer + 11, resp2, 32);

        byte tempCS[1] = {0};
        tempCS[0] = GeneratePacketChecksum(tempBuffer, 43);
