...
#define pin_power A1
#define max_ref_volt 964 // Adjustment, since in fact 4.2 V from the battery will be equal to 1.05 V on the divider
#define max_voltage_battery 4180 // mv battery max (for safety against overcharging)
...
void getVoltage()
{
  batteryVoltage = map(analogRead(pin_power), 0, 1023, 0, max_voltage_battery);
}

void testLed()
{
  // Low Power
  if (batteryVoltage < min_volt)
  {
...
void sendCharge()
{
  int charge = map(batteryVoltage, 0, max_voltage_battery, 0, max_charge_value);
