...
#define pin_power A1
#define max_ref_battery 976 // Adjustment, since in fact 4.2 V from the battery will be equal to 1.05 V on the divider
#define max_voltage_battery 4200 // mv battery max 
#define min_voltage_battery 3100 // mv battery min 
...
void getVoltage()
{
  batteryVoltage = map(analogRead(pin_power), 0, max_ref_battery, 0, max_voltage_battery);
}
...
void sendCharge()
{
  int charge = map(batteryVoltage, min_voltage_battery, max_voltage_charging_battery, 0, max_charge_value);
